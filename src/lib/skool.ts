...
export async function fetchPosts(buildId: string, page: number): Promise<SkoolPost[]> {
  ...
  return postTrees.map((tree: Record<string, unknown>) => {
    ...
    return {
      ...
      createdAt: parseTimestamp((post.createdAt as string) || ''),
      updatedAt: parseTimestamp((post.updatedAt as string) || ''),
...
function flattenComments(children: Record<string, unknown>[], parentId: string | null = null): Comment[] {
  ...
  for (const child of children || []) {
    ...
    results.push({
      ...
      content: (metadata?.content as string) || '',
      upvotes: (metadata?.upvotes as number) || 0,
      createdAt: parseTimestamp((c.created_at as string) || (c.createdAt as string) || ''),
      updatedAt: parseTimestamp((c.updated_at as string) || (c.updatedAt as string) || ''),
